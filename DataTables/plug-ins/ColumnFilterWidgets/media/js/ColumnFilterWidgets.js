...
(function($) {
	...
	*/
	var ColumnFilterWidgets = function( oDataTableSettings ) {
		var me = this;
		var sExcludeList = '';
		me.$WidgetContainer = $( '<div class=""><div class="column-filter-widgets "></div></div>' );
		...
		$.each( oDataTableSettings.aoColumns, function ( i, oColumn ) {
			var $WidgetElem = $( '<div class="column-filter-widget column medium-2 small-6 right "></div>' );
	...
	*/
	var ColumnFilterWidget = function( $Container, oDataTableSettings, i, widgets ) {
		...
		widget.$Select = $( '<select></select>' ).addClass('widget-' + widget.iColumn).change( function() {
			...
			sText = $( '<div class="row">' + sSelected + '</div>' ).text();
			$TermLink = $( '<a class="filter-term" href="#" ></a> <br />' )
